// turn_left(), turn_right(), stationary_turn_left(), stationary_turn_right(), turn_left_no_delay()
             ...
             
{
  ...
  Serial.print("inside turn_left function");
    while( 1 )
      ...
      left = 1;
      //right = right ;
      analogWrite(pin_speed_R, startSpeed_R); // Change to startSpeedL//
             ...
             
{
  ...
  Serial.print("inside turn_right function");
    ...
    while( 0 != right)
      ...
      analogWrite(pin_speed_L, startSpeed_L); // Change to startSpeedL//
      ...
      read_IR_sensors(); // may need to change to LDR4
        if (LDR2 + threshold <= LDR3) // stop turning right when sense black
        {
          ...
          Serial.print("\n stop turning right 1-1-1-1-1-1-1-1\n");
            if (1 == returning_to_start_flag)
            {
              Serial.println("returning_to_start_flag = 1");
              delay(200);
            } 
          break;
        }
...
int turn_left_no_delay()
{
  analogWrite(pin_speed_R, 103);  // Mar. 23, 103 was the speed that worked for corner_hoppers
  analogWrite(pin_speed_L, 0);
  return 0;
}
